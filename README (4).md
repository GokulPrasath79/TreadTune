...
**Apollo Tyres Ltd × Vredestein Tyres — Pre-Development Project**
Status: Prototype / Proof of Concept · Target: FY27
...
7. [Prototype (This Repository)](#7-prototype-this-repository)
8. [Production Technology Stack](#8-production-technology-stack)
9. [Feasibility Analysis](#9-feasibility-analysis)
10. [Risks &amp; Mitigations](#10-risks--mitigations)
11. [Roadmap](#11-roadmap)
12. [Deliverables Checklist](#12-deliverables-checklist)
13. [Limitations of the Current Prototype](#13-limitations-of-the-current-prototype)
...
flowchart LR
    subgraph Input
        A[CAD Drawing<br/>.dwg / .dxf]
    end

    subgraph "Core Engine"
        B[CAD Import &amp; Parsing]
        C[Pattern Feature<br/>Identification]
        D[Land:Sea<br/>Analysis Engine]
        E[Tuning / Optimization<br/>Module]
        F[Multipitch<br/>Scaling Engine]
    end

    subgraph Output
        G[CAD Export<br/>.dwg / .dxf]
        H[Bitmap Export<br/>.png]
        I[Report / Log<br/>.json]
    end

    A --> B --> C --> D --> E
    E -- "not within tolerance" --> D
    E -- "within tolerance" --> F
    F --> G
    F --> H
    F --> I
```
...
```mermaid
flowchart LR
    A[Tread Pattern Drawing<br/>I/P: CAD file] --> B[Pattern Feature<br/>Identification]
    B --> C[Analysis<br/>Land : Sea]
    C --> D{Need Land:Sea<br/>tuning?}
    D -- No --> G[Proceed to<br/>Multipitch Scaling]
    D -- Yes --> E[Tune Land:Sea]
    E --> F{Land:Sea %<br/>OK?}
    F -- Not OK --> E
    F -- OK --> G
    G --> H[Finish &amp; Export<br/>O/P: CAD file, Bitmap]
```
...
- [x] Tool with user-friendly interface (input & output) — prototype delivered
- [ ] Native AutoCAD drawing file input/output — pending CAD-API integration (Phase 1–3)
- [x] Bitmap file output — implemented in prototype
- [x] System architecture diagram — this document, §4
- [x] Algorithm documentation — this document, §6
...
2. Open `treadtune_prototype.html` directly in a modern browser (Chrome/Edge/Firefox) — no build step or server required.
3. Upload a tread pattern image, or click **"Use synthetic sample"**.
4. Work through Steps 01–06 in the left panel: detect features → analyze ratio → tune → scale to multi-pitch → export.
